...
class MultiObjectTracker():
    ...
    :param centerPoint: used for determining which object is closest to the centerPoint
    """
    ...
    def __init__(self, maxDisappeared=1, maxMatchDistance=20, maxIgnoreDistance=15, minAppearance=1, isCVTrackerOn=False, cvTrackerType='MEDIANFLOW', debugMode=False, centerPoint=(640,360)):
        assert cvTrackerType in MultiObjectTracker.availableTrackerTypes, 'cvTrackerType not in availableTrackerTypes'
        self.nextObjectID = 0
        self.objects = OrderedDict()
        self.validObjects = OrderedDict() #an obj is valid if it continuously appears more than minAppearance times from YOLO update
        ...
        self.isTrackingCenter=False
        self.cvTracker=None
        self.closestObjectID=None
        self.centerPoint=centerPoint
        self.YOLOResults=deque([])
    ...
    def enableCVTracking(self,frame):
        self.isTrackingCenter=True
        self.cvTracker = getattr(cv2, self.trackerFunctionName)
        bbox=self.minmaxToXywh( self.validObjects[self.closestObjectID][1])
        self.cvTracker.init(frame, bbox)
    ...
    def minmaxToXywh(self, bbox):
        return (bbox[0],bbox[1],bbox[2]-bbox[0], bbox[3]-bbox[1])

    def removeDuplicate(self, inputCentroids, rects):
        newRects = []
        newCentroids = []
        skipList = []
        addedList = []
    ...
    def update(self,frame):
        """
        Update object info continuously
        :return:
        """
        if len(self.YOLOResults) > 0:
            self.processYOLOResults(self.YOLOResults.popleft())
            if self.isTrackingCenter:
                bbox = self.minmaxToXywh(self.validObjects[self.closestObjectID][1])
                self.cvTracker.init(frame, bbox)
        else:
            if self.isTrackingCenter:
                ok, bbox = self.tracker.update(frame)
                bbox=self.xywhToMinmax(bbox)
                _,  _, label, _ = self.objects[self.closestObjectID]
                cX = int((bbox[0] + bbox[2]) / 2.0)
                cY = int((bbox[1] + bbox[3]) / 2.0)
                centroid = (cX, cY)
                self.updateObject(centroid, bbox, label,frame=None, objectID=self.closestObjectID)
                #TODO: handle object missing

    def findClosestObjectToCenter(self):
        validObjectIDs = list(self.validObjects.keys())
        validObjectInfo = list(self.validObjects.values())
        minDistance=1469 #half of the diagonal in 2K :)
        minPosition=-1
        for i in range(0,len(validObjectInfo)):
            currentCentroid=validObjectInfo[i][0]
            currentDistance=(((self.centerPoint[0]-currentCentroid[0])**2)+((self.centerPoint[0]-currentCentroid[0])**2))
            if currentDistance<minDistance:
                minDistance=currentDistance
                minPosition=i
        if minPosition==-1:
            raise Exception("In MultiObjectTracker.findClosestObjectToCenter: minDistance not updated")
        return validObjectIDs[minPosition]

    def updateFromYOLO(self,rects):
        """
        Update object lists and match detection results to objects. Return objects at the end.
        This is called when YOLO updates
        :param rects: A list of lists containing bounding box as a tuple of (startX, startY, endX, endY) and the detected object label
        """
        self.YOLOResults.append(rects)

    def processYOLOResults(self, rects):
        """
        register and match objects from YOLO detection results
        :param rects: A list of lists containing bounding box as a tuple of (startX, startY, endX, endY) and the detected object label
        """
        # [(label,conf,bbox)]

...
class MultiObjectTracker():
    ...
    def update(self, rects):
        ...
        #Remove close rects from input, take major label and average centroids
        newRects = []
        newCentroids = []
        skipList = []
        ...
        for i in range(0, len(inputCentroids)):
            if i in skipList:
                continue
            D = dist.cdist(np.array([inputCentroids[i]]), inputCentroids)
            centroidGroups.append([i, []])
            isTakingAverage=False
            for j in range(0, len(D)):
                if i==j or j in skipList:
                    continue
                if (D[j]<self.maxIgnoreDistance):
                    isTakingAverage=True
                    centroidGroups[currentIndex][1].append(j)
                    skipList.append(j)
                else:
                    break
            if isTakingAverage:
                #Take average
                xAvg=inputCentroids[centroidGroups[currentIndex][0]][0]
                yAvg = inputCentroids[centroidGroups[currentIndex][0]][1]
                redCount=0
                blueCount=0
                for itemIndex in centroidGroups[currentIndex][1]:
                    xAvg+=inputCentroids[itemIndex][0]
                    yAvg+=inputCentroids[itemIndex][1]
                    labelVal=rects[itemIndex][1]
                    if labelVal=='Red':
                        redCount+=1
                    elif labelVal=='Blue':
                        blueCount+=1
                xAvg=xAvg/len(centroidGroups[currentIndex][1])+1
                yAvg=yAvg/len(centroidGroups[currentIndex][1])+1
                deltaX=round(xAvg-inputCentroids[centroidGroups[currentIndex][0]][0])
                deltaY=round(yAvg-inputCentroids[centroidGroups[currentIndex][0]][1])
                bboxVal, labelVal=rects[centroidGroups[currentIndex][0]]
                if redCount<blueCount:
                    labelVal='Blue'
                else:
                    labelVal='Red'
                xmin, ymin, xmax, ymax=bboxVal
                xmin+=deltaX
                xmax+=deltaX
                ymin+=deltaY
                ymax+=deltaY
                newBbox=(xmin,ymin,xmax,ymax)
                newRects.append([newBbox, labelVal])
                newCentroids.append((xAvg, yAvg))
            else:
                itemIndex=centroidGroups[currentIndex][0]
                newRects.append(rects[itemIndex])
                newCentroids.append(inputCentroids[itemIndex])
            currentIndex+=1
            isTakingAverage=False
